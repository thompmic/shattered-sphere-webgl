...
3. ~~Export `22.blend` → `.glb`~~ ✅ **DONE — `export/hero.draco.glb`, 643 KB, 4 draw calls,
   65,267 tris, shatter math verified against a reference render.** Full detail in **§5f**.
   (The first export was 1.13 MB / 112,883 tris; **§12.8** cut the wire cage and with it 42%
   of the file.)
...
⚠️ **Two clauses of that have since moved on** — the rest still holds. **Mobile was built**
once this became a portfolio (§13.2/§13.3), and **the copy is written**: zero placeholder
chips remain (changelog 2026-08-16).
...
│   ├── hero.draco.glb            643 KB — **the one to ship**
│   ├── hero.glb                  5.75 MB — uncompressed fallback
...
| File | Size | Use |
|---|---|---|
| `export/hero.draco.glb` | **643 KB** | **ship this** — Draco compressed, 8.7× smaller |
| `export/hero.glb` | 5.75 MB | uncompressed fallback if Draco ever misbehaves |
...
#### The 4 meshes (4 draw calls, 65,267 tris)

> The sizes above and the cage figure below are **post-§12.8**. The original export was
> 1.13 MB with a 63,488-tri cage; dropping `CAGE_SUBSURF` from 2 to 1 is what changed them.
...
| `wire_cage` | 15,872 | `lines` | `Sphere` with its geo-node instancers disabled — subsurf+decimate+wireframe only. Was 63,488 and **the heaviest object in the scene** until §12.8 cut `CAGE_SUBSURF` to 1 |
...
| 4 | Responsive | **Landscape/desktop first.** Mobile is a separate later phase — do not spend effort on it now. ⚠️ **Superseded — see §13.2/§13.3:** once this became a portfolio, mobile stopped being deferrable and was built 2026-08-12 |
| 5 | Copy | User will rewrite all text later. **Leave the Figma placeholder copy in for now**, but drive every string from one content file so a rewrite is a single-file edit. ✅ **Done — the rewrite happened 2026-08-16:** every string in `content.js` is real, zero placeholder chips remain |
...
build is clean at ≈381 KB gzipped JS (three 187 + r3f 176 + entry 12 + Stage 6) + the
643 KB model.
...
| `src/scroll.js` | one Lenis instance; exposes a plain `scroll` object — `target` raw, **`value`** damped, and **the scene must read `value`** (§12.6) — read inside `useFrame` rather than React state, so scrolling never re-renders the canvas tree |
...
- The lower sections put body copy straight over the subject; it reads, but only just.
  Either darken behind the text or push the subject aside as `scroll.value` grows.
...
### 13.2 What a personal portfolio needs — and where each piece stands

This table was written 2026-08-12 as a list of what was *missing*. It is kept as the checklist
but **re-statused against the code as of 2026-08-23** — most of it landed in §13.3 and the
2026-08-16 content pass.

| Piece | Status | Note |
|---|---|---|
| Hero / identity | ✅ | carries the user's real name and role (`identity` in `content.js`), not "Studio One.Zero" |
| About | ✅ | three paragraphs, written from the user's own four interests |
| Projects / work | ✅ | three real projects with role, stack and year. **No thumbnails** — still the one thing this section could gain |
| Skills / stack | ✅ | five groups, every item drawn from one of the three projects |
| Experience or education | ❌ | still absent, and still optional — nobody has decided whether the site wants it |
| Interests | ✅ | four items |
| Contact + socials | ✅ | real email, real GitHub and LinkedIn URLs. Nothing points at `#` |
| Resume/CV download | ❌ | `contact.resume` is `null`, so the button is hidden. Drop a PDF in `public/` and set the path to turn it on |
| Meta: real `<title>`, description, OG image, favicon | ✅ | all present, `og:image` added 2026-08-16 |
| Accessibility pass | ⚠️ | skip link, `:focus-visible`, `sr-only` labels and reduced-motion are done. **Contrast has not been measured and there is no canvas-free fallback** |
| Mobile | ✅ built, ⚠️ unseen | §13.3 — works, but has never been opened on a real phone (§12.5) |
| Deployment | ✅ | live on Netlify, auto-deploys on push to `main` |
